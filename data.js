...
function getPrecipIconURL(chance) {
  if (chance <= 20) {
    return "https://w7.pngwing.com/pngs/318/505/png-transparent-weather-forecasting-computer-icons-cloud-public-domain-icons-cloud-orange-weather-forecasting.png";
  } else if (chance <= 40) {
    return "https://w7.pngwing.com/pngs/166/384/png-transparent-cloud-sun-sunny-weather-weather-flat-icon-thumbnail.png";
  } else {
    return "https://www.citypng.com/public/uploads/preview/cloud-rain-icon-transparent-png-701751695039291vvyoeptyc3.png";
  }
}
    ...
    for (let i = 1; i <= 7; i++) {
      $("#High" + i + "W1").html(
        data.daily.temperature_2m_max[i - 1] +
          "°F" +
          " <img class='cell-icon' src='https://i.pinimg.com/736x/0a/70/58/0a705876a6c34795d0ab4ec81ec0d5b1.jpg'>"
      );
      $("#Wind" + i + "W1").html(
        data.daily.wind_speed_10m_max[i - 1] +
          " mph" +
          " <img class='cell-icon' src='https://media.lordicon.com/icons/wired/lineal/812-wind.svg'>"
      );
      ...
      $("#Pre" + i + "W1").html(
        chance + "%" + " <img class='cell-icon' src='" + iconURL + "'>"
      );
    }
  }
}
...
function getWeatherGov(gridX, gridY) {
  $.ajax({
    url:
      "https://api.weather.gov/gridpoints/ILN/" +
      gridX +
      "," +
      gridY +
      "/forecast",
...
function displayWeatherGov(data) {
  if (data.properties.periods != null) {
    for (let i = 1; i <= 7; i++) {
      ...
      $("#Pre" + i + "W2").html(
        chanceW2 + "%" + " <img class='cell-icon' src='" + iconURL_W2 + "'>"
      );
    }
  }
}
...
// --------------------
// LOG FUNCTIONS (NO PHP)
// --------------------
function addLog(Gemini) {
  $.ajax({
    url: "https://weathersite.lovellmb.workers.dev/addLog",
    method: "POST",
    contentType: "application/json",
    data: JSON.stringify({
      request: zip,
...
function getLog(index) {
  $.ajax({
    url: "https://weathersite.lovellmb.workers.dev/getLog",
    method: "GET",
    contentType: "application/json",
  })
    .done(function (data) {
  ...
  $.ajax({
    ...
    .done(function (data) {
      if (!(zipcode === "") && date === "") {
        ...
        for (let i = start; i < data.result.length && i >= 0; i = i + move) {
          if (
            zipcode === data.result[i].request &&
            data.result[i].timestamp.includes(date)
          ) {
            historyIndex = i;
            getLog(historyIndex);
            break;
